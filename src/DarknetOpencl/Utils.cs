...
namespace DarknetOpencl
{
    public unsafe class utils
    {
        ...
        public unsafe int[] read_intlist(string gpu_list, int* ngpus, int d)
        {
            unsafe
            {
                int* gpus = 0;
    ...
    void sorta_shuffle(XArrayIntPtr arr, int n, int size, int sections)
    {
        int i;
        for (i = 0; i < sections; ++i)
        {
            int start = n * i / sections;
            int end = n * (i + 1) / sections;
            int num = end - start;
            shuffle(arr + (start * size), num, size);
        }
    }

    void shuffle(void* arr, int n, int size)
    {
        int i;
    ...
    int read_all_fail(int fd, char* buffer, int bytes)
    {
        int n = 0;
        while (n < bytes)
        {
    ...
    int write_all_fail(int fd, char* buffer, int bytes)
    {
        int n = 0;
        while (n < bytes)
        {
    ...
    void read_all(int fd, char* buffer, int bytes)
    {
        int n = 0;
        while (n < bytes)
        {
    ...
    void write_all(int fd, char* buffer, int bytes)
    {
        int n = 0;
        while (n < bytes)
        {
